...
public:
    ...
    SubscribeAndPublish()
    {
        ...
        _pub = nh.advertise<geometry_msgs::Twist>("/cmd_vel", 1000);
        
        
        _sub = nh.subscribe("/odom",1000,&SubscribeAndPublish::callback,this);
	PDR_sub = nh.subscribe("/PDR_position",10,&SubscribeAndPublish::PDRcallback,this);
...
private:
    ros::NodeHandle nh;
    ros::Publisher _pub;
    ros::Subscriber _sub,PDR_sub;
    
    
    
};
void SubscribeAndPublish::PDRcallback(const geometry_msgs::Point::ConstPtr& PDRmsgs)
{
	yd = PDRmsgs -> y;
	xd = PDRmsgs -> x;
	ROS_INFO_STREAM("goal_x "<<xd<<" goal_y "<<yd);
}


void SubscribeAndPublish::callback(const nav_msgs::Odometry::ConstPtr& msgs)
{
        
        double theta;
        float kit=.0001,kdt=0,kpt=0.5;
